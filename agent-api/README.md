...
## Local Auth & Schema Setup (Development Only)

To enable local user authentication (email/password) and tracking tables (runs, datasets, artifacts) before migrating to Supabase:

1. Apply the local schema file (after containers are up so Postgres is reachable):
   ```sh
   psql 'postgresql://ai:ai@localhost:5432/ai?sslmode=disable' -f db/schema_local.sql
   ```
2. Set environment variables (in your shell or docker compose):
   ```sh
   AUTH_SECRET=dev-super-secret            # Replace with a strong random string
   AUTH_ACCESS_MINUTES=15                  # Access token TTL
   AUTH_REFRESH_DAYS=7                     # Refresh token TTL
   AUTH_PBKDF2_ITERATIONS=80000            # Increase for stronger hashing (trade-off: speed)
   ```
3. Restart the `api` service if you changed env vars.

Auth endpoints now available:
- `POST /v1/auth/register` (email, password, optional display_name)
- `POST /v1/auth/login`
- `GET /v1/auth/me` (requires `Authorization: Bearer <access_token>`)

Returned JSON includes `access_token` & `refresh_token`. Use the access token in subsequent requests to associate `user_id` internally with runs and sessions.

Future migration to Supabase:
- Replace `public.users` with `auth.users`.
- Keep `profiles`, `agent_runs`, `datasets`, `artifacts` (adjust foreign keys to `auth.users.id`).
- Enable Row Level Security (RLS) + policies per table.
- Move artifact & dataset file storage paths to Supabase Storage bucket URLs.
...
We use Claude Sonnet as the default model, please export the `ANTHROPIC_API_KEY` environment variable to get started.
...
1. **Configure for Production**
  * Ensure your production environment variables (e.g., `ANTHROPIC_API_KEY`, database connection strings) are securely managed. Most cloud providers offer a way to set these as environment variables for your deployed service.
