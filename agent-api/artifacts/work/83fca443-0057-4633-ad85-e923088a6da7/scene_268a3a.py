...
class GenScene(Scene):
    def construct(self):
        # Load the dataset
        df = pd.read_csv("/static/datasets/dataset_20251218_023137/20150601_CaitlynJennerCity.csv")
        
        # Create title
        title = Text("Caitlyn Jenner City Data", font_size=36)
        title.to_edge(UP)
        ...
        if len(df.columns) > 0:
            ...
            header = Text(header_text, font_size=24, color=YELLOW)
            header.next_to(title, DOWN, buff=1)
            ...
            for i in range(rows_to_show):
                row_data = []
                for col in headers:
                    value = str(df.iloc[i][col])[:10]  # Truncate long values
                    row_data.append(value)
                
                row_text = " | ".join(row_data)
                row = Text(row_text, font_size=20)
                data_group.add(row)
            
            data_group.arrange(DOWN, buff=0.3)
            data_group.next_to(header, DOWN, buff=0.5)
