...
class GenScene(Scene):
    def construct(self):
        # Create sample data since we can't load external files
        math_avg = 66.1
        reading_avg = 69.2
        writing_avg = 68.1
        ...
        pie_group = VGroup(female_sector, male_sector)
        
        # Labels for pie chart
        gender_title = Text("Gender Distribution", font_size=36, color=WHITE)
        gender_title.next_to(pie_group, UP, buff=1)
        
        female_label = Text(f"Female: {female_count}", font_size=24, color
